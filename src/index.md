...
# Streaming Score Bias Dashboard

This dashboard compares IMDb and TMDb scores across Amazon, Netflix, Hulu, and Disney+.  
Use the controls to change the dataset, year, and minimum votes.
...
const temporalToggleInput = Inputs.toggle({label: "Expand Temporal View", value: false});
...
<div class="grid grid-cols-1">
  <div class="grid grid-cols-4 gap-4">
    ...
    <div class="card" style="background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); border: 1px solid #333; padding: 24px;">
      <h2 style="color: #fff; margin-top: 0; margin-bottom: 20px; font-size: 18px; font-weight: 600;">Titles (filtered)</h2>
      <div style="color: #fff; font-size: 48px; font-weight: 700; line-height: 1; margin-bottom: 20px; text-shadow: 0 2px 8px rgba(0,0,0,0.3);">${stats.total.toLocaleString("en-US")}</div>
      <div style="background: rgba(255,255,255,0.05); border-radius: 8px; padding: 12px; margin-bottom: 16px; backdrop-filter: blur(10px);">
        <div style="color: #999; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; font-weight: 600;">Platform Breakdown</div>
      ...
      <div style="margin-top: 16px; display: flex; justify-content: center;">${resize(width => titlesPie(width))}</div>
    </div>
    <div class="card">
      <h2>${temporalExpanded ? "Temporal Gap (Lines) ▼ (Expanded)" : "Temporal Gap (Lines) ◀ (Minimized)"}</h2>
      ${temporalToggleInput}
      <div style="margin-top: 12px;">${resize(width => temporalExpanded ? createChart(width, "Temporal Gap (Lines)") : createTemporalMini(width))}</div>
    </div>
    <div class="card">
      <h2>Δ(IMDb − TMDb)</h2>
      <div>Amazon: ${stats.meanDiffAmazon.toFixed(2)}</div>
      <div>Others: ${stats.meanDiffOther.toFixed(2)}</div>
    </div>
  </div>
</div>
...
<div class="grid grid-cols-1">
  <div class="card">
    <h2>${temporalExpanded ? (vizType + " (minimized)") : vizType}</h2>
    <div>${temporalExpanded ? resize(width => vizType === "Bias Detector (Scatter)" ? createTemporalMini(width) : createTemporalMini(width)) : resize(width => createChart(width))}</div>
  </div>
</div>